...
void timer3Init(void)
{
	...
	TCNT3H = 0;						// reset TCNT3
	TCNT3L = 0;
	ETIMSK = (1<<TOIE3);						// enable TCNT3 overflow
}
...
void timer3SetPrescaler(unsigned char prescale)
{
	// set prescaler on timer 2
	TCCR3B = ((TCCR3B & ~TIMER_PRESCALE_MASK) | prescale);
}

unsigned short timer0GetPrescaler(void)
{
	// get the current prescaler setting
	return (pgm_read_word(TimerPrescaleFactor+((TCCR0) & TIMER_PRESCALE_MASK)));
}

unsigned short timer1GetPrescaler(void)
{
	// get the current prescaler setting
...
unsigned short timer3GetPrescaler(void)
{
	// get the current prescaler setting
...
void timer1PWMInitICR(unsigned short topcount)
{
	// set PWM mode with ICR top-count
	TCCR1A &= ~(1<<WGM10);
	TCCR1A |= (1<<WGM11)|(1<<WGM12)|(1<<WGM13);
	// set top count value
	ICR1H = (unsigned char)(topcount>>8);
	ICR1L = (unsigned char)topcount;
...
void timer1PWMASet(unsigned short pwmDuty)
{
...
void timer1PWMBSet(unsigned short pwmDuty)
{
	...
	//			pwmDuty should be in the range 0-511 for 9bit PWM
	//			pwmDuty should be in the range 0-1023 for 10bit PWM
	OCR1BH = (pwmDuty>>8);		// set the high 8bits of OCR1B
	OCR1BL = (pwmDuty&0x00FF);	// set the low 8bits of OCR1B
}

void timer1PWMCSet(unsigned short pwmDuty)
{
	...
	//			pwmDuty should be in the range 0-511 for 9bit PWM
	//			pwmDuty should be in the range 0-1023 for 10bit PWM
	OCR1CH = (pwmDuty>>8);		// set the high 8bits of OCR1C
	OCR1CL = (pwmDuty&0x00FF);	// set the low 8bits of OCR1C
}

void timer3PWMInit(unsigned char bitRes)
{
	// configures timer1 for use with PWM output
...
void timer3PWMInitICR(unsigned short topcount)
{
	// set PWM mode with ICR top-count
	TCCR3A &= ~(1<<WGM30);
	TCCR3A |= (1<<WGM31);
	...
	// set top count value
	ICR3H = (unsigned char)(topcount>>8);
	ICR3L = (unsigned char)topcount;
...
void timer3PWMASet(unsigned short pwmDuty)
{
	// set PWM (output compare) duty for channel A
...
void timer3PWMBSet(unsigned short pwmDuty)
{
	// set PWM (output compare) duty for channel B
	...
	//			pwmDuty should be in the range 0-511 for 9bit PWM
	//			pwmDuty should be in the range 0-1023 for 10bit PWM
	OCR3BH = (pwmDuty>>8);		// set the high 8bits of OCR3B
	OCR3BL = (pwmDuty&0x00FF);	// set the low 8bits of OCR3B
}

void timer3PWMCSet(unsigned short pwmDuty)
{
	// set PWM (output compare) duty for channel B
	...
	//			pwmDuty should be in the range 0-511 for 9bit PWM
	//			pwmDuty should be in the range 0-1023 for 10bit PWM
	OCR3CH = (pwmDuty>>8);		// set the high 8bits of OCR3C
	OCR3CL = (pwmDuty&0x00FF);	// set the low 8bits of OCR3C
}
